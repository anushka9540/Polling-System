<template>
  <Header></Header>
  <div class="flex justify-center bg-center" :style="{
            backgroundImage: `url(${backgroundImage})`,
            backgroundPosition: 'center bottom -60px',
            backgroundRepeat: 'no-repeat'
          }">
    <div class="w-full max-w-[480px] relative mt-[-16px] sm:w-[100%] sm:px-4 lg:w-[480px] sm:p-8 lg:mt-[-40px]"
      ...
      <div class="relative z-10 pl-10 pr-10 bg-white shadow-lg rounded-b-2xl">
        <form @submit.prevent="handleLogin">
          ...
          <div class="relative mb-4">
            <label for="password"
              class="block text-sm text-[#5a6978] mb-[4px] mt-[20px] font-sans text-[15px] font-medium">Password:</label>
            ...
            <span @click="togglePassword" class="absolute transform -translate-y-1/2 cursor-pointer right-3 top-[54px]">
              <i class="fa-solid fa-eye" :style="{ color: showPassword ? '#19b7ea' : '#858585' }"></i>
            </span>
          </div>
  
          <div class="text-center mb-[15px] mt-[19px]">
            <a href="#" class="text-[13.5px] text-[#5a6978] font-medium">Forgot Password?</a>
          </div>
          <div class="flex items-center w-[260px] p-2 -mb-2 bg-gray-50 border border-gray-300 rounded-[5px]">
  <label class="relative flex items-center cursor-pointer">
    <input v-model="isHuman" type="checkbox"
      class="hidden peer" />
  ...
  <img :src="hCaptchaImg" class="h-[43px] ml-[80px]" />
          ...
          <button :disabled="!isHuman" class="w-full py-[7px] rounded-[250px] shadow-md text-[20px] mt-3" :class="
                    isHuman ? 'bg-[#19b7ea] text-white' : 'bg-[#dadada] text-[#999]'
                  ">
            Log In
          </button>
        </form>
        ...
        <div class="flex flex-col items-center space-y-4">
          <button
            ...
            <img :src="facebookicon" class="mr-5 w-[30px] h-[25px] text-[#1b3e85]" />
            Log in with Facebook*
          </button>
  
          <button
            class="w-full h-[40px] bg-white border border-gray-400 text-gray-600 rounded-[8px] flex items-center  hover:bg-gray-100">
            <img :src="googleicon" class="ml-[87px] w-[27px] h-[22px]" />
            <p class="pl-[25px]">Log in with Google*</p>
          </button>
  
          <p class="pt-1 text-left text-gray-400 text-[10px] leading-loose p-3 font-normal">
...
const { form, errors, handleLogin, showPassword, togglePassword } = useAuth();
